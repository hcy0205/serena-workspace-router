...
def test_router_stdio_allows_first_activation_without_meta_when_project_is_explicit(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    project_root = _create_project(tmp_path, "stdio-project-without-meta")
    runtime_dir = tmp_path / "runtime"
    coordination_dir = tmp_path / "coordination"
    bootstrap_dir = tmp_path / "bootstrap"
    bootstrap_dir.mkdir(parents=True, exist_ok=True)
        ...
        with router_stderr.open("w", encoding="utf-8") as errlog:
            async with stdio_client(
                ...
                async with ClientSession(read_stream, write_stream) as session:
                    ...
                    assert inspect_payload["projectPath"] == str(project_root)
